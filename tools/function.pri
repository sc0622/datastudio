...
defineTest(generateQmlTypes) {
    ...
    $$compiler($$group($${name}.clean), $$removedir($$outdir/))

    ## - copy files

    ## -- qmlplugindump
    item = qmlplugindump
    $$compiler($$group($${name}.$${item}), \
        $$copyfile($$[QT_INSTALL_PREFIX]/bin/$${item}$$execsuffix(), $$outdir/))

    ## -- depfiles
    for (file, depfiles) {
        item = $$replace(file, \*|\.|/|:, '_')
        $$compiler($$group($${name}.$$item), $$copyfile($$file, $$outdir/))
    }

    ## copy default files of Qt
    $$compiler($$group($${name}.$$item), \
        $$group($$[QT_INSTALL_PREFIX]/bin/windeployqt$$execsuffix() \
            $$outdir))
    $$compiler($$group($${name}.$$item), \
        $$copyfile($$[QT_INSTALL_PREFIX]/plugins/platforms/qminimal$$dylibsuffix(), \
            $$outdir/platforms/))

    ## generate qmltypes

    commands = $$outdir/qmlplugindump$$execsuffix() -nonrelocatable \
        $$uri $$version $$dst > $$qmltypes
    $$compiler($${name}.generate, $$group($$commands))
