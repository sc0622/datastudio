...
J_EXTERN_C J_ATTR_EXPORT void *CreateComponent(void* attemper)
{
    ...
    return static_cast<IJComponent *>
            (new ComSimulatorNav(reinterpret_cast<IJAttempter *>(attemper)));
}

ComSimulatorNav::ComSimulatorNav(IJAttempter *attemper)
...
ComSimulatorNav::~ComSimulatorNav()
{
    if (d_ui) {
        d_ui->deleteLater();
        d_ui = 0;
...
bool ComSimulatorNav::loadInterface()
{
    bool result = true;

    // 订阅消息
    d_attempter->notifier().beginGroup(this)
            .append("parserChanged", &ComSimulatorNav::onParserChanged)
            .append("query_loading_deep", &ComSimulatorNav::queryLoadingDeep)
            .endGroup();

    return result;
}

bool ComSimulatorNav::updateInterface()
{
    bool result = true;
...
void ComSimulatorNav::releaseInterface()
{
    // 取消订阅消息
    d_attempter->notifier().beginGroup(this)
...
void* ComSimulatorNav::queryInterface(const std::string &iid, unsigned int ver)
{
...
std::string ComSimulatorNav::componentName() const
{
    return QStringLiteral("com_simulator_nav").toStdString();
}

std::string ComSimulatorNav::componentDesc() const
{
    return QStringLiteral("com_simulator_nav").toStdString();
}

void ComSimulatorNav::attach()
{
    // 订阅消息
    d_attempter->notifier().beginGroup(this)
            .endGroup();
...
void *ComSimulatorNav::createWindow(void *parent, const std::string &objectName)
{
    Q_UNUSED(parent);
    Q_UNUSED(objectName);
    ...
    //
    d_ui = new ComSimulatorNavUi(*d_attempter);
...
bool ComSimulatorNav::commandSink(void *sender, const std::string &domain, const std::string &objectName,
...
bool ComSimulatorNav::messageSink(IJComponent *sender, const std::string &id, JWPARAM wParam, JLPARAM lParam)
{
    Q_UNUSED(sender);
    Q_UNUSED(id);
    Q_UNUSED(wParam);
...
std::string ComSimulatorNav::observerId() const
{
    return componentName();
}

JLRESULT ComSimulatorNav::onParserChanged(const std::string &id, JWPARAM wParam, JLPARAM lParam)
{
    Q_UNUSED(id);
    Q_UNUSED(wParam);
    Q_UNUSED(lParam);
...
JLRESULT ComSimulatorNav::queryLoadingDeep(const std::string &id, JWPARAM wParam, JLPARAM lParam)
{
    Q_UNUSED(id);
    Q_UNUSED(lParam);
    ...
    QObject *comboBoxLoadingDeep = (QObject *)d_attempter->mainWindow()->queryObject(
                "combo_loading_deep@group_property@page_simulator@" + componentName());
