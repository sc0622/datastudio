    ...
    : QWidget(parent)
{
    ...
    d_icdTree->setTreeMode(Icd::CoreTreeWidget::TreeModeSimulator);
    ...
    connect(d_icdTree, &Icd::CoreTreeWidget::itemPressed, this, [=]
            (QStandardItem *item){
        //
        jframeNotifier().sendMessage("treeItemPressed@com_simulator", (JWPARAM)item);
    });
    connect(d_icdTree, &Icd::CoreTreeWidget::itemClicked, this, [=]
            (QStandardItem *item){
        //
        jframeNotifier().sendMessage("treeItemClick@com_simulator", (JWPARAM)item);
    });
    connect(d_icdTree, &Icd::CoreTreeWidget::itemUnloaded, this, [=]
            (QStandardItem *item, QStandardItem *itemTable){
        //
        jframeNotifier().sendMessage("treeItemUnloaded@com_simulator",
    ...
    connect(d_icdTree, &Icd::CoreTreeWidget::channelBound, this, [=]
        ...
        //
        jframeNotifier().sendMessage("channelBound@com_simulator",
                                     (JWPARAM)item, (JLPARAM)&channelId);
    });
    connect(d_icdTree, &Icd::CoreTreeWidget::channelUnbound, this, [=]
            (QStandardItem *item, const QString &channelId){
        //
        jframeNotifier().sendMessage("channelUnbound@com_simulator",
                                     (JWPARAM)item, (JLPARAM)&channelId);
    });
    connect(d_icdTree, &Icd::CoreTreeWidget::channelChanged, this, [=]
            (QStandardItem *item, const QString &channelId){
        //
        jframeNotifier().sendMessage("channelChanged@com_simulator",
                                     (JWPARAM)item, (JLPARAM)&channelId);
    });
    connect(d_icdTree, &Icd::CoreTreeWidget::unbindItem, this, [=]
            (QStandardItem *item){
        //
        jframeNotifier().sendMessage("unbindItem@com_simulator", (JWPARAM)item);
    });
}
...
bool MainNavWidget::init()
{
    ...
    // deep
    int deep = 0;
    if (jframeNotifier().sendMessage("query_loading_deep@com_simulator_nav",
    ...
    //
    d_icdTree->setBindTableType(Icd::CoreTreeWidget::BindOnlySend);
...
void MainNavWidget::setPlay(bool running)
{
    ...
    if (jframeNotifier().sendMessage("setPlay@com_simulate",
