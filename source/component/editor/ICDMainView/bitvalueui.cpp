...
BitValueUI::BitValueUI(IJAttempter &attempter, QWidget *parent)
...
void BitValueUI::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void* BitValueUI::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void BitValueUI::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int BitValueUI::originalType() const
{
    if (!q_old) {
...
// 数据录入完成
void BitValueUI::slotEditFinished()
{
    // 校验数据
...
// 编辑文本变更
void BitValueUI::slotTextChanged(const QString& text)
{
...
// 范围限制状态变更
void BitValueUI::slotRangeStateChanged(bool enable)
{
    QCheckBox *check = qobject_cast<QCheckBox *>(sender());
    QStringList lstRange = QString(q_data->dataRange().c_str()).split("~");
    if (std::string::npos == q_data->dataRange().find("~")) {
...
// 表点击
void BitValueUI::slotTableItemPressed(QStandardItem *item)
{
    Q_UNUSED(item);
...
void BitValueUI::init()
{
    ...
    if (q_spinLength) {    // 长度
        ...
        q_spinLength->setValue(q_data->bitLength());
    }
    QStringList lstRange = QString(q_data->dataRange().c_str()).split("~");
    if (q_spinMin) {    // 范围下限
...
// 启/停用信号槽
void BitValueUI::enableConnection(bool enable)
{
...
// 启/停用界面控制按钮
void BitValueUI::enableOptionButton(bool enable)
{
    if (q_btnConfirm) {
...
// 特征表菜单处理
void BitValueUI::dealAction(QAction *action)
{
    if (!action)
    {
        return;
...
// 向特征值表插入一条数据
void BitValueUI::insertValueOne(int index, const _Eigenvalue &data)
{
    int column = 0;
