...
{
public:
    explicit DataManegement(IJAttempter* attemper);
    virtual ~DataManegement();
...
protected:
    // 消息响应回调
    JLRESULT notifyRespond(const std::string &id, JWPARAM wParam, JLPARAM lParam);
...
protected:
    ...
    */
    ...
    // 保存规则数据
    bool saveRule(const QString &keys,
    ...
    // 查询内存数据
    JLRESULT querySingleElement(const std::string &id,
                                JWPARAM wParam, JLPARAM lParam);
    // 查询加载数据源
    JLRESULT queryDataSource(const std::string &id,
                             JWPARAM wParam,
                             JLPARAM lParam);
    // 查询子表上限
    JLRESULT queryMaxTableCount(const std::string &id,
                                JWPARAM wParam,
                                JLPARAM lParam);
    // 保存拷贝数据
    JLRESULT saveCopyData(const std::string &id,
                          JWPARAM wParam, JLPARAM lParam);
    // 将内存数据保存到数据库
    JLRESULT saveDatabase(const std::string &id,
                          JWPARAM wParam, JLPARAM lParam);
    // 将内存数据保存到文件
    JLRESULT saveFile(const std::string &id,
                      JWPARAM wParam, JLPARAM lParam);
    // 更新数据库配置信息
    JLRESULT dataSourceChanged(const std::string &id,
                               JWPARAM wParam, JLPARAM lParam);
    // 查询数据库错误信息
    JLRESULT queryDBError(const std::string &id,
                          JWPARAM wParam, JLPARAM lParam);
    // 查询顶层表
