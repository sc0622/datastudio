...
double FrameCodeItem::data() const
{
    ...
    icd_uint64 value = 0;
    memcpy(&value, buffer, std::min<int>(sizeof(value), bufferSize()));
    return value;
}
...
void FrameCodeItem::setData(double data)
{
    ...
    memcpy(buffer, &value, std::min<int>(sizeof(value), bufferSize()));
}
