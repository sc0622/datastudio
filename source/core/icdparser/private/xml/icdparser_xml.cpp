...
XmlParser::XmlParser(const std::string &filePath, Icd::Parser *parser)
...
bool XmlParser::convert2Other(const Icd::VehiclePtrArray &vehicles, std::vector<int> &data)
{
    Q_UNUSED(vehicles);
    Q_UNUSED(data);
    Q_ASSERT(false);
...
bool XmlParser::convert2Core(const std::vector<int> &data, Icd::VehiclePtrArray &vehicles)
{
    Q_UNUSED(vehicles);
    Q_UNUSED(data);
    Q_ASSERT(false);
