...
JsonParser::JsonParser(const std::string &filePath, Parser *parser)
...
bool JsonParser::save(const Icd::VehiclePtrArray &vehicles)
{
    ...
    return JJson::setValue(d_filePath, "", rootJson);
}
...
bool JsonParser::save(const TablePtrArray &tables)
{
    ...
    return JJson::setValue(d_filePath, "", tablesJson);
}
...
bool JsonParser::convert2Other(const Icd::VehiclePtrArray &vehicles, std::vector<int> &data)
{
    Q_UNUSED(vehicles);
    Q_UNUSED(data);
    Q_ASSERT(false);
...
bool JsonParser::convert2Core(const std::vector<int> &data, Icd::VehiclePtrArray &vehicles)
{
    Q_UNUSED(vehicles);
    Q_UNUSED(data);
    Q_ASSERT(false);
...
{
    return JJson::parse(d_filePath, value, create);
}
