 ...
 */
bool DBAccess::init(const std::string &file)
{
    if (!loadDBInfo(file, d->q_dbInfo)) {
        return false;
    }

    // construct database
    const QString connetionName("com.td.icd.dbaccess.sql");
 ...
 * @return 执行结果， true：成功；false：失败
 */
bool DBAccess::loadDBInfo(const std::string &file, stDBInfo& dbInfo)
{
#if 1
    TiXmlDocument doc(file);

    if (!doc.LoadFile()) {
        d->q_error = QStringLiteral("failed to read database"
                                    " config！\r\n<%1> \r\n[%2]")
                .arg(file.c_str()).arg(doc.ErrorDesc());
        d->q_errorFunc = __FUNCTION__;
        return false;
    }
    TiXmlNode* root = doc.FirstChild("jframe");
    if (!root) {
        d->q_error = QStringLiteral("can not find database config section！");
        d->q_errorFunc = __FUNCTION__;
        return false;
    }
    ...
    settings.beginGroup("DBCFG");
    dbInfo.driver = settings.value("DRIVER").toString().toStdString();
    dbInfo.server = settings.value("SERVER").toString().toStdString();
    dbInfo.database = settings.value("DATABASE").toString().toStdString();
    dbInfo.user = settings.value("UID").toString().toStdString();
    dbInfo.pwd = settings.value("PWD").toString().toStdString();
