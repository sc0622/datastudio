...
//
const unsigned short table_crc16[256] = {
...
bool doCheck(const Icd::TablePtr &table, const char *buffer)
{
    if (!table || !buffer) {
        return false;
    }
...
bool checkData(const TablePtr &table, int fileHeaderSize, QFileDevice *source,
               QFileDevice *target)
{
    ...
    qint64 dataSize = source->size() - fileHeaderSize;
    if (dataSize < table->bufferSize()) {
        qWarning() << QString("file \"%1\" is tool small!").arg(source->fileName());
        return false;
    }
    ...
    do {
        bufferSize = qMin<qint64>(mapSize, dataSize - mapOffset);
        buffer = source->map(mapOffset, bufferSize);
        if (!buffer) {
            result = false;
            break;
        }
        ...
        for (int i = 0, section = 0; i < bufferSize; ++i) {
            ...
            if (currentIndex == tableSize) {
                if (doCheck(table, parseBuffer)) {
                    target->write(parseBuffer, tableSize);
                }
                currentIndex = 0;
            }
        }
        source->unmap(buffer);
    } while (mapOffset < dataSize);
