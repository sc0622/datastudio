...
SerialSendWidget::SerialSendWidget(QWidget *parent)
    : QWidget(parent)
    , d_worker(Icd::WorkerPtr())
{
    ...
    connect(d_buttonSwitch, &QPushButton::clicked,
            this, &SerialSendWidget::onButtonSwitchClicked);
...
SerialSendWidget::~SerialSendWidget()
{

}

void SerialSendWidget::updateUi(const Icd::WorkerPtr &worker)
{
    if (d_worker) {
    ...
    connect(d_worker.get(), &Icd::Worker::closed,
            this, &SerialSendWidget::onWorkerClosed);
    connect(d_worker->workerSend().get(), &Icd::WorkerSend::started,
            this, &SerialSendWidget::onWorkerStarted);
    connect(d_worker->workerSend().get(), &Icd::WorkerSend::stopped,
            this, &SerialSendWidget::onWorkerStopped);
...
void SerialSendWidget::onWorkerClosed()
{
    d_buttonSwitch->setText(tr("Start"));
}

void SerialSendWidget::onWorkerStarted()
{
    d_buttonSwitch->setText(tr("Stop"));
}

void SerialSendWidget::onWorkerStopped()
{
    d_buttonSwitch->setText(tr("Start"));
}

void SerialSendWidget::onButtonSwitchClicked()
{
    if (!d_worker) {
        return;
    }
