...
void ArrayEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void*ArrayEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void ArrayEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int ArrayEdit::originalType() const
{
    if (!q_old) {
...
void ArrayEdit::slotEditFinished()
{
...
void ArrayEdit::slotTextChanged(const QString& text)
{
...
// 启/停用信号槽
void ArrayEdit::enableConnection(bool enable)
{
