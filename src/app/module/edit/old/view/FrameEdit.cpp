...
void FrameEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void* FrameEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void FrameEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int FrameEdit::originalType() const
{
    if (!q_old) {
...
// 数据录入完成
void FrameEdit::slotEditFinished()
{
...
// 编辑框文本变更
void FrameEdit::slotTextChanged(const QString& text)
{
...
// 启/停用信号槽
void FrameEdit::enableConnection(bool enable)
{
