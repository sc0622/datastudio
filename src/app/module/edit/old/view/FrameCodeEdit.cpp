    ...
    : ObjectEdit(parent)
{
    ...
    addFormRow(QStringLiteral("长度："), comboLength);

    comboData_ = new QComboBox(this);
    addFormRow(QStringLiteral("帧数据："), comboData_);
...
bool FrameCodeEdit::init()
{
    // length
    comboLength->clear();
    comboLength->addItem(QStringLiteral("1字节"), GlobalDefine::frame8);
    comboLength->addItem(QStringLiteral("2字节"), GlobalDefine::frame16);
    comboLength->addItem(QStringLiteral("4字节"), GlobalDefine::frame32);
