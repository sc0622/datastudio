...
void FrameCodeEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void*FrameCodeEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void FrameCodeEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int FrameCodeEdit::originalType() const
{
    if (!q_old) {
...
void FrameCodeEdit::slotEditFinished()
{
...
void FrameCodeEdit::slotTextChanged(const QString& text)
{
...
// 初始化帧码长度下拉框
void FrameCodeEdit::initBoxLenght()
{
    // 固定，只有1、2、4字节可选
    if (!q_boxLength) {
        return;
...
// 初始化帧数据下拉框
void FrameCodeEdit::initBoxData()
{
    if (!q_boxData) {
        return;
    }
...
// 启/停用信号槽
void FrameCodeEdit::enableConnection(bool enable)
{
