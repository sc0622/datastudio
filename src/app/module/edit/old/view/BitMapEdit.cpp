    ...
    : ObjectEdit(parent)
{
    ...
    editDefault_->setPlaceholderText(QStringLiteral("二进制"));
    editDefault_->setToolTip(QStringLiteral("二进制！"));
    addFormRow(QStringLiteral("默认值："), editDefault_);
...
bool BitMapEdit::validate()
{
    ...
    if (editDefault_->text().length() > spinBitCount_->value()) {
        ...
        setStatus(QStringLiteral("默认值超出起止范围！"));
        return false;
    } else {
        editDefault_->setProperty("highlight", false);
    }
    ...
    if (error) {
        ...
        setStatus(QStringLiteral("值含义中不能有此符号‘,’"));
        return false;
    } else {
