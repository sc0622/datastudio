...
#ifndef _MAX_BIT_COUNT
#define _MAX_BIT_COUNT 64
#endif
    ...
    : ObjectEdit(parent)
{
    ...
    q_spinStart->setRange(0, _MAX_BIT_COUNT - 1);
    q_spinLength->setRange(1, _MAX_BIT_COUNT);
    regExp.setPattern("[0-1]{,64}");
    q_edtDefault->setValidator(new QRegExpValidator(regExp));
    q_edtDefault->setMaxLength(_MAX_BIT_COUNT);
...
void BitMapEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void*BitMapEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void BitMapEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int BitMapEdit::originalType() const
{
    if (!q_old) {
...
// 数据录入完成
void BitMapEdit::slotEditFinished()
{
    ...
    if (tedt) {
        ...
        if (spin == q_spinStart) {
            // 更新长度上限
            q_spinLength->setMaximum(_MAX_BIT_COUNT - spin->value());
...
// 编辑文本变更
void BitMapEdit::slotTextChanged(const QString& text)
{
...
void BitMapEdit::init()
{
    ...
    if (q_spinStart) {  // 起始位
        ...
        q_spinStart->setMaximum(_MAX_BIT_COUNT - 1);
    ...
    if (q_spinLength) {    // 长度
        ...
        q_spinLength->setMaximum(_MAX_BIT_COUNT);
...
// 启/停用信号槽
void BitMapEdit::enableConnection(bool enable)
{
...
// 获取特征值表数据
std::unordered_map<double, std::string> BitMapEdit::tableValues() const
{
