...
void BufferEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void* BufferEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void BufferEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int BufferEdit::originalType() const
{
    if (!q_old) {
...
// 数据录入完成
void BufferEdit::slotEditFinished()
{
...
void BufferEdit::slotTextChanged(const QString& text)
{
...
// 启/停用信号槽
void BufferEdit::enableConnection(bool enable)
{
