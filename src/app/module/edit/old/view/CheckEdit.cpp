...
void CheckEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void*CheckEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void CheckEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int CheckEdit::originalType() const
{
    if (!q_old) {
...
void CheckEdit::slotEditFinished()
{
...
void CheckEdit::slotTextChanged(const QString& text)
{
...
// 启/停用信号槽
void CheckEdit::enableConnection(bool enable)
{
