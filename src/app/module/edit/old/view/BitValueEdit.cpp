...
void BitValueEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void* BitValueEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void BitValueEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int BitValueEdit::originalType() const
{
    if (!q_old) {
...
// 数据录入完成
void BitValueEdit::slotEditFinished()
{
    // 校验数据
...
// 编辑文本变更
void BitValueEdit::slotTextChanged(const QString& text)
{
...
// 范围限制状态变更
void BitValueEdit::slotRangeStateChanged(bool enable)
{
...
// 表点击
void BitValueEdit::slotTableItemPressed(QStandardItem *item)
{
    Q_UNUSED(item);
...
// 启/停用信号槽
void BitValueEdit::enableConnection(bool enable)
{
