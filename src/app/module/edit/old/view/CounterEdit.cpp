...
void CounterEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void*CounterEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void CounterEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int CounterEdit::originalType() const
{
    if (!q_old) {
...
void CounterEdit::slotEditFinished()
{
...
void CounterEdit::slotTextChanged(const QString& text)
{
...
// 启/停用信号槽
void CounterEdit::enableConnection(bool enable)
{
