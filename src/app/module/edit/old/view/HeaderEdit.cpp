...
void HeaderEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void *HeaderEdit::uiData() const
{
    return (void*)&q_data;
}

// 切换数据类型
void HeaderEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int HeaderEdit::originalType() const
{
    if (!q_old) {
...
void HeaderEdit::slotEditFinished()
{
...
void HeaderEdit::slotTextChanged(const QString& text)
{
    // 校验数据
...
// 启/停用信号槽
void HeaderEdit::enableConnection(bool enable)
{
