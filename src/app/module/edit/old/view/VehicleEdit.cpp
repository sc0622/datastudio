...
void VehicleEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void *VehicleEdit::uiData() const
{
    return (void*)&q_data;
}

// 编辑框文本变更
void VehicleEdit::slotTextChanged(const QString &text)
{
...
// 数据录入完成
void VehicleEdit::slotEditFinished()
{
...
// 启/停用信号槽
void VehicleEdit::enableConnection(bool enable)
{
...
bool VehicleEdit::dataValid()
{
    ...
    if (q_data.sName.empty()) {
        ...
        editStatus()->setText(QStringLiteral("已存在同名机型！"));
        palette.setColor(QPalette::Base, Qt::red);
        q_edtName->setPalette(palette);
        q_edtName->setFocus();
