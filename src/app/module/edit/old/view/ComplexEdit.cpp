...
void ComplexEdit::setUIData(const _UIData &data)
{
    if (!data.data) {
        return;
    }
...
void* ComplexEdit::uiData() const
{
    return (void*)&q_data;
}

// 界面操作按钮可用
void ComplexEdit::changeDataType(int type)
{
    q_data->setType(type);
...
// 原始数据类型
int ComplexEdit::originalType() const
{
    if (!q_old) {
...
// 数据录入完成
void ComplexEdit::slotEditFinished()
{
...
void ComplexEdit::slotTextChanged(const QString& text)
{
...
// 启/停用信号槽
void ComplexEdit::enableConnection(bool enable)
{
